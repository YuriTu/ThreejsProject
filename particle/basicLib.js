...
* 定义一个可视图形的基本属性
*/
    const DisplayClass = Class.extend({
...
* 交互对象
*/
    const InteractiveClass = DisplayClass.extend({
	                                                                            init: function (option) {
		                                        this._super(option);
		                                        this.eventListener = {};
},
		                                        ...
		                                        for (let i = 0; i < this.eventListener[type].length; i++) {
// 删除指定的监听器
...
* Sprite 容器
*/
    const ObjectContainerClass = InteractiveClass.extend({
...
// 有事件监听则遍历执行
				                                        for (let j = 0, arr = this.children[i].eventListener[type]; j < arr.length; j++) {
...
// 重新清理鼠标悬浮下的对象数组
		                                        ...
		                                        const tempArr = [];
		                                        for (let i = 0; i < this.hoverChildren.length; i++) {
...
*/
    const Stage = ObjectContainerClass.extend({
...
//
		                                        const context = this;
		                                        const batchAddMouseEventListener = function (el, evArr) {
			                                        for (let i = 0; i < evArr.length; i++) { // console.log(evArr[i])
				                                        el.addEventListener(evArr[i], function (param, i) {
					                                        return function (e) {
						                                        let x = e.clientX - param.canvas.offsetLeft,
							                                        y = e.clientY - param.canvas.offsetTop;
						                                        if (!!param.eventListener[evArr[i]]) {
							                                        for (let j = 0; j < param.eventListener[evArr[i]].length; j++) {
		                                        ...
		                                        const batchAddKeyEventListener = function (el, evArr) {
...
* }
*/
    const Sprite = ObjectContainerClass.extend({
	                                                                            init: function (ctx, option) {
...
this.height < this.maxHeight ? this.maxHeight / this.height : 1
		                                        ...
		                                        this.dragFunc = function (_x, _y) {
			                                        let offsetX = _x - context.dragPos.x,
...
// 判断粒子的生存时间
	                                                                            aging: function (dt) {
		                                        for (let i = 0; i < this.$private.particles.length; ) {
			                                        const p = this.$private.particles[i];
			                                        p.age += dt;
			                                        if (p.age > p.life) {
				                                        this.kill(i);
} else {
		                                        ...
		                                        for (const i in this.$private.particles) {
			                                        this.$private.particles[i].acceleration = this.gravity;
}
},
	                                                                            applyEffectors: function () {
		                                        for (const j in this.effectors) {
			                                        const apply = this.effectors[j].apply;
			                                        for (const i in this.$private.particles) {
...
*/
	                                                                            render: function (ctx) {
		                                        for (const i in this.$private.particles) {
			                                        let p = this.$private.particles[i],
...
*/
    const ParticleBlock = Class.extend({
